...
ブラウザだけで複数の WAV トラックを同期再生し、必要な範囲を確認・切り出せるシンプルな音声エディターです。全トラック共通の再生範囲は秒数入力で指定し、コピー・切り取り用の編集範囲はトラックごとに波形上のドラッグで個別に指定できます。切り取りではトラックの長さを変えず、選択区間を無音にします。トラックごとの音量・ミュートと、全体の音量・ローパスフィルター・リバーブを再生中にも調整でき、選択中のトラックを WAV として保存できます。音声ファイルはサーバーへ送信されません。
